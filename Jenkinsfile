  ...
  stages {
   ...
   stage('Restarting POD') {
      steps {
        ...
        script {
            try{
                sh 'ssh digesetuser@148.213.1.131 microk8s.kubectl rollout restart deployment prueba-j --kubeconfig=/home/digesetuser/.kube/config'
		sh 'ssh digesetuser@148.213.1.131 microk8s.kubectl rollout status deployment prueba-j --kubeconfig=/home/digesetuser/.kube/config'
              }catch(error){
         }
	}
      }
