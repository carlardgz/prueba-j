  ...
  stages {
   ...
   stage('Restarting POD') {
      steps {
        ...
        script {
            try{
                sh 'sudo ssh digesetuser@148.213.1.131 microk8s.kubectl rollout restart deployment prueba-j --kubeconfig=/root/.kube/config'
		sh 'sudo ssh digesetuser@148.213.1.131 microk8s.kubectl rollout status deployment prueba-j --kubeconfig=/root/.kube/config'
              }catch(error){
         }
	}
      }
